...
describe("Thinking-step Markdown transform", () => {
	...
	it("renders collapsed latest and summary latest-five with original numbering", () => {
		expect(transformThinkingSteps(seven, enabled("collapsed"), context())).toBe(
			"**Step 7:** Label 7",
		);
		expect(transformThinkingSteps(seven, enabled("summary"), context())).toBe(
			[
				"- **Step 3:** Label 3",
				"- **Step 4:** Label 4",
				"- **Step 5:** Label 5",
	...
	it("renders every label and original body only for finalized/restored Expanded content", () => {
		const source = "# First\nOriginal **Markdown**.\n```\n# opaque\n```\n# Second\n$$\nx\n$$";
		const expected = [
			"### Step 1: First",
			"Original **Markdown**.\n```\n# opaque\n```",
		...
		expect(transformThinkingSteps(source, enabled("expanded"), context())).toBe(expected);
		expect(
	...
	it("sizes labels at narrow and wide widths without splitting graphemes or Markdown", () => {
		...
		const narrow = transformThinkingSteps(
			input,
			enabled("collapsed"),
			context({ availableWidth: 19 }),
		);
		expect(narrow).toMatch(/^\*\*Step 1:\*\* Family .*…$/);
	...
	it("caps labels by display width independently of source length", () => {
		...
		const renderedLabel = output.replace(/^\*\*Step 1:\*\* /, "");
...
describe("Thinking-step public capability registration", () => {
	...
	it("registers exactly one callback and reads current config on every chain call", () => {
		...
		expect(callback?.("# One\n# Two", context())).toBe("**Step 2:** Two");
		config = enabled("summary");
		expect(callback?.("# One\n# Two", context())).toBe("- **Step 1:** One\n- **Step 2:** Two");
	});
