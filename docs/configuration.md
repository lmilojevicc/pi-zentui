...
**Rail** and **Tree** use only Pi's public `registerMarkdownTransformer` API and only transform assistant-thinking Markdown. Rail shows every safely parsed structural label in chronological order; Tree shows the latest five in their original order. Both show labels only, with no visible numbering or bodies. Their compact title is simply `Thinking`, with bold emphasis and no heading semantics. Ordinary Unicode connectors and neutral `·` marks identify settled labels, while only the latest streaming label uses `•` and bold emphasis. Labels come directly from top-level headings, top-level list items, and blank-line-separated prose; fenced code, Mermaid, display math, and indented content remain opaque.

Pi 0.84 or newer provides that public API. On Pi 0.80.5–0.83, `/zentui` preserves an enabled Rail/Tree preference, shows `Using native thinking — requires Pi 0.84 or newer`, and leaves native thinking byte-for-byte unchanged. Public Markdown transforms run for new and streaming content, restored messages, width changes, and other public rebuilds. Pi has no public invalidation API for already settled same-width transcript entries, so settings changes appear there only when Pi otherwise rebuilds that thinking content.
