...
Thinking steps use Pi's public Markdown-transformer API on Pi 0.84 or newer. Rail shows every safely parsed structural label in chronological order; Tree shows the latest five in their original order. Both use the compact title `Thinking`, labels only, and no visible numbering. The title is bold, settled labels use neutral `·` marks, and only the latest streaming label uses `•` plus bold. On Pi 0.80.5–0.83, an enabled Rail/Tree preference is preserved but native thinking remains byte-for-byte unchanged. Mode changes affect new, streaming, restored, resized, or otherwise rebuilt thinking; already settled same-width transcript entries cannot always be publicly invalidated.
